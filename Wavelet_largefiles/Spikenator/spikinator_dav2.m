...
%     mvbar=rmsv(xsm); %local threshold (RMS)
...
%                   regardless of whether or not the local thresholding (cleanthr)
%                   came back positive, since this threshold could be distorted by
...
% x = x(10000:18000);
% x(4000:end) = x(4000:end) + 1;

if amp>0                        % daveadded
    y=x(find(~isnan(x))).';
    spk=spk-y;    
elseif ((amp == 0) && (fade == 0))
    y=x;                    % Leave data as is with gaps
%     y=x(find(~isnan(x))).';
    spk=0;
elseif (fade > 0)        % Apply fade
    xnan = isnan(x);
